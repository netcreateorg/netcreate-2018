...
/// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
const { ERR_UR } = require('./_ur/_lib/declare-errors').EXIT_CODES;
const LOG = require('./_ur/_lib/prompts').makeTerminalOut('UR', 'TagBlue');
const ERROUT = require('./_ur/_lib/prompts').makeTerminalOut('ERR', 'TagRed');
const DIE = (...args) => {
  ERROUT(`\x1b[93m${args.join()}\x1b[0m`);
  ERROUT(new Error(`UR Process Terminated (${ERR_UR})`).stack);
  process.exit(ERR_UR);
};
/// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
const DBG = true;
