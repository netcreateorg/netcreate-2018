...
/*/ MOD.Hook('INITIALIZE', function () {
  if (TEST('call')) {
    ...
    UDATA.HandleMessage('TEST_CALL', data => {
      ...
      if (typeof data.source === 'string' && data.source === 'DevUnisysJSX')
        TEST.Pass('callHndlrDataProp');
      data.source = 'DevUnisysLogic-Return';
      if (!data.stack) data.stack = [];
      data.stack.push('TRI-1');
  ...
  if (TEST('remote')) {
    UDATA2.HandleMessage('REMOTE_CALL_TEST', data => {
      if (data && Array.isArray(data.results)) {
        ...
        if (DBG.handler)
          console.log(PR2, 'got REMOTE_CALL_TEST message with data', data);
    ...
    for (let i = 0; i < 5; i++) {
      setTimeout(function () {
        ...
        UDATA2.Call(
          ...
          if (data !== undefined && Array.isArray(data.stack))
            TEST.Pass('serverData');
          if (data.stack && data.stack.length === 2 && data.stack[1] === 'SRV_01')
            TEST.Pass('serverDataAdd');
          if (data.me && data.me === 'DevUnisysLogic:MOD2.Start')
            TEST.Pass('serverReturn');
...
/*/ MOD2.Hook('START', function () {
  ...
  function delayed_send() {
    ...
    UDATA.NetCall('NET_CALL_TEST', call).then(d => {
...
/*/ MOD.Hook('START', function () {
  ...
  /*/ if (TEST('remote')) {
    ...
    // Do the call test!
    UDATA.LocalCall('REMOTE_CALL_TEST', { melon: 'logicmelon' }).then(data => {
      if (data && data.melon && data.cat) TEST.Pass('remoteData');
      if (data.melon === 'logicmelon_ack' && data.cat === 'calico')
