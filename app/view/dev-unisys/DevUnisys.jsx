  ...
  componentDidMount() {
    ...
    if (TEST('call')) {
      ...
      this.AppCall('TEST_CALL', { source: 'DevUnisysJSX' })
        // test data return
        .then(data => {
          if (data && data.source && data.source === 'DevUnisysLogic-Return')
            TEST.Pass('callDataReturn');
          if (data && data.extra && data.extra === 'AddedData')
            TEST.Pass('callDataAdd');
          if (
            data &&
            data.multi &&
            data.stack &&
