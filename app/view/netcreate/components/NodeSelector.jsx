  ...
  /*/
    UDATA.HandleMessage('NODE_EDIT', data => {
      ...
      if (
        ...
        if (isLocked)
          console.warn(
            'NodeSelector.NODE_EDIT denied because isLocked',
            isLocked
          );
      }
    });
  ...
  /*/
    UDATA.HandleMessage('EDGE_UPDATE', data => {
      ...
      /* EDGE_UPDATES are triggered under two circumnstances:
        ...
        */
      let sourceID =
        typeof data.edge.source === 'number'
          ? data.edge.source
          : data.edge.source.id;
      let targetID =
        typeof data.edge.target === 'number'
          ? data.edge.target
          : data.edge.target.id;
      let updatedNodeIDs = [sourceID, targetID];
      if (
        updatedNodeIDs.includes(currentNodeID) &&
        !this.state.edgesAreLocked
      ) {
        if (DBG)
          console.log('NodeSelector: EDGE UPDATE: Calling SOURCE_SELECT!');
        UDATA.LocalCall('SOURCE_SELECT', { nodeIDs: [currentNodeID] });
      }
    });
    /// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    UDATA.HandleMessage('EDGE_NEW_CANCEL', data => {
      if (data.nodeID === this.state.formData.id) {
        this.setState({ edgesAreLocked: false }, () => {
          ...
          UDATA.LocalCall('SOURCE_SELECT', {
            nodeIDs: [this.state.formData.id]
          });
        });
      } else {
      ...
      this.AppState('NCDATA').nodes.find(function (node) {
        if (node.id !== formData.id) {
          if (node.label) {
            ...
            console.log(
              'error processing node: ' + node.id + ' in netc-app.js\n'
            );
            return false;
          }
        }
...
/*/
  ...
  dateFormatted() {
    ...
    var date =
      today.getMonth() + 1 + '/' + today.getDate() + '/' + year.substr(2, 4);
...
/*/
  ...
  requestEditNode(nodeID) {
    this.NetCall('SRV_DBLOCKNODE', { nodeID: nodeID }).then(data => {
      if (data.NOP) {
        ...
        console.log(
          `SERVER SAYS: lock success! you can edit Node ${data.nodeID}`
        );
  ...
  editNode() {
    ...
    this.NetCall('SRV_REQ_EDIT_LOCK', { editor: EDITORTYPE.NODE }).then(
      data => {
...
/*/ onCancelButtonClick() {
    ...
    if (this.state.isBeingEdited) {
      ...
      if (originalNode === undefined) {
        ...
        // unlock
        this.NetCall('SRV_DBUNLOCKNODE', {
          nodeID: this.state.formData.id
        }).then(data => {
          if (data.NOP) {
            console.log(`SERVER SAYS: ${data.NOP} ${data.INFO}`);
          } else if (data.unlocked) {
        ...
        >
          ...
          <FormGroup row hidden={nodeDefs.provenance.hidden}>
            <Col sm={3} style={{ hyphens: 'auto' }} className="pr-0">
              <Label
                for="provenance"
                className="tooltipAnchor small text-muted"
              >
          ...
          >
            ...
            <p>
              ...
              {formData.label} (ID {formData.id}). {citation.text}. Last
              accessed at {this.dateFormatted()}.
            </p>
          </div>
          <br />
          ...
          <FormGroup className="text-right" style={{ paddingRight: '5px' }}>
            ...
            <div
              hidden={
                isLocked || isStandalone || isBeingEdited || formData.id === ''
              }
            ...
            >
              ...
              >
                {nodeIsLockedMessage}
                <span hidden={!isAdmin}>
                  &nbsp;<b>ADMINISTRATOR ONLY</b>: If you are absolutely sure
                  this is an error, you can force the unlock:
                  <br />
                  <Button
